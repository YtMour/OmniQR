...
当前仓库已完成 OmniQR 工具箱 MVP 的主要本地闭环：普通文本/链接、WiFi、名片二维码均可在 H5/App 基础链路中本地生成，生成后自动保存记录，并支持二维码预览、复制内容和图片导出回退；记录页支持搜索、筛选、收藏、结构化详情、删除；设置页支持生成偏好、二维码尺寸、8 款二维码风格、每款 10 款造型、定位角独立造型、固定自定义颜色、语言切换、JSON 导出下载、JSON 导入去重、数据清理、隐私政策和免责声明；二维码同内容重复生成也会刷新随机样式；扫码链接会先确认安全风险，首页也提供从相册识别二维码入口。

Android 真机已完成相机扫码、相册保存、二维码识别等主链路验证，当前未发现明显问题。发布前仍建议按发布检查清单做一次回归复测。

详细完成度和后续复测项见 [docs/IMPLEMENTATION_STATUS.md](./docs/IMPLEMENTATION_STATUS.md)。

...
- `OmniQR-Uniapp/components/QrPreview.vue`：二维码预览组件。只接收已经生成的内容，空状态显示“点击生成后显示二维码”；已生成状态显示放大且紧凑的品牌外框、随机角落 `OmniQR` 水印和扫码安全的方块、软角、圆角、点块造型，定位角保持标准方形。
...
5. 二维码图片导出由 `OmniQR-Uniapp/common/qrExport.ts` 负责，导出图包含品牌外框、角落 `OmniQR` 水印、二维码块造型和定位角造型。

注意：`QrPreview.vue` 内部样式使用 `omniqr-preview-*` 私有类名前缀，避免被页面级 `.qr-placeholder`、`.preview-icon` 等旧样式污染。

二维码预览网格必须只使用真实二维码矩阵尺寸，`gridColumnStart` / `gridRowStart` 必须保持整数模块位置；外框与二维码之间的紧凑留白只能通过矩阵容器 padding 或整体尺寸控制，不能用 1.5 这类小数 quiet zone 参与 CSS Grid 行列计算，否则会导致模块错位和大片空白。
...
- 二维码预览空状态不应与旧占位模块重叠。
- 生成后的二维码应带 OmniQR 品牌外框、角落水印和轻量模块造型；外框与二维码之间不应出现大面积空白，定位角必须保持标准方形，模块不得使用空心、菱形、细条、十字或过度缩小等影响扫码识别的形态。
